...
候选着生成保留邻近两格内的空位，排序优先即时成五和必须防守的着法。
安静候选着最多 16 个。这是研发基线，不保证完备搜索或棋力。
当前没有置换表，已有可测试的局面哈希为后续实现准备。
...
未来 TT 必须区分规则、尺寸、行棋方和模型上下文，切换权重时不能复用旧评估。
...
启用 C++ 异常以保留限时搜索的撤销语义；目前不用 pthreads、SIMD 或共享内存。
